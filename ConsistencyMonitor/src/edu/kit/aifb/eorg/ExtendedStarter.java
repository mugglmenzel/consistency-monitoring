...
import edu.kit.aifb.eorg.cloudpolling.SimpleDBMultiFilePoller;
import edu.kit.aifb.eorg.cloudpolling.SimpleDBPoller;
...
import edu.kit.aifb.eorg.cloudwriter.SimpleDBMultiFileWriter;
import edu.kit.aifb.eorg.cloudwriter.SimpleDBWriter;
			...
			if (line.startsWith("id")) {
				...
				if (line.equals(id)) {
					...
					if (line.equalsIgnoreCase("collector")) {
						...
						writer.runWriter(params);
						return;
					} else if (line.equalsIgnoreCase("simpledbwriter")) {
						line = config.remove(0);
						line = line.substring(line.indexOf(":") + 1);
						start = Long.parseLong(line);
						params = new String[7];
						...
						params[6] = awsprivate;
						countdown(start);
						SimpleDBWriter writer = new SimpleDBWriter();
						writer.runWriter(params);
						return;
					} else if (line.equalsIgnoreCase("simpledbmultifilewriter")) {
						line = config.remove(0);
						line = line.substring(line.indexOf(":") + 1);
						String alternateBucket = line;
						line = config.remove(0);
						...
						params[8] = alternateFile;
						countdown(start);
						SimpleDBMultiFileWriter writer = new SimpleDBMultiFileWriter();
						writer.runWriter(params);
						return;
						...
						poller.runPoller(params);
						return;
					} else if (line.equalsIgnoreCase("simpledbpoller")) {
						line = config.remove(0);
						line = line.substring(line.indexOf(":") + 1);
						start = Long.parseLong(line);
						params = new String[7];
						...
						params[6] = awsprivate;
						countdown(start);
						SimpleDBPoller poller = new SimpleDBPoller();
						poller.runPoller(params);
						return;
					} else if (line.equalsIgnoreCase("simpledbmultifilepoller")) {
						line = config.remove(0);
						line = line.substring(line.indexOf(":") + 1);
						boolean useStandardFile = Boolean.parseBoolean(line);
						line = config.remove(0);
						...
						params[9] = alternateFile;
						countdown(start);
						SimpleDBMultiFilePoller poller = new SimpleDBMultiFilePoller();
						poller.runPoller(params);
						return;

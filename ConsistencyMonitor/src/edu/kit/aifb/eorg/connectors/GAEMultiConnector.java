 ...
 */
public final class GAEMultiConnector {
	...
	public final static String readFromGAE(final String key) {
		...
		try{
    	    ...
    	    String timestamps = split[0];
    	    for (int i=1; i<split.length-1;i++){
    	    	timestamps = timestamps+";"+split[i];
    	    }	
    	    String classVariable = split[split.length-1];
    	    long endTime = new Date().getTime();
    	    long latency = (endTime-startTime)/2;
    	    System.out.println("Data = "+timestamps+"\nKlassenvariable: "+classVariable+"\nLatency GET in ms: " + latency);
    	    file.writeBytes(timestamps+";Klassenvariable;"+classVariable+";Latency GET in ms:;" + latency + "\n;");
    		rd.close();
